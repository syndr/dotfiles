...
export PATH=/usr/bin:/usr/local/bin:$HOME/.local/bin:$HOME/bin:/home/linuxbrew/.linuxbrew/bin:$HOME/.npm-global/bin:$PATH
...
export MCFLY_PROMPT="❯"
if [ -x /usr/bin/mcfly ]; then
    echo "Using mcfly from /usr/bin/mcfly"
    eval "$(/usr/bin/mcfly init zsh)"
else
    eval "$(mcfly init zsh)"
fi
